...
//
// BRDF is the result of playing with Unreal Engine code, labs, and this paper:
// https://perso.telecom-paristech.fr/jthiery/cours/igr202/files/rendu2.pdf
// (description - p. 28-38, Fresnel - p. 39-42, distribution - p. 43-45, geometric - p. 46-47)
//
...
void main()
{
	...
	// Assume all the color maps are sRGB & convert to linear space.
	// DDS can have sRGB format, though, but it seems gli does not provide a smooth way to know about & convert it yet.
	...
	for (int i = 0; i < LightCount; i++)
	{
		...
		// For the case without shadow sampler
		//vec3 tc = ShadowTC[i].xyz / ShadowTC[i].w;
		//float shadow = texture(Light[i].ShadowTexture, tc.xy).x;
		//shadow = tc.z > shadow ? 1.0 : 0.0;

		// For the case without shadow sampler
		//float shadow = texture(Light[i].ShadowTexture, sc.xyz).x;
		//shadow = sc.z > shadow ? 1.0 : 0.0;
#endif
		...
		// Fake some ambient occlusion because we don't have secondary light bounces.
		// Needs AO texture.
		float A = 0.5 * ao + 0.5;

		// Light attenuation (remember we are limited to spot lights at the moment).
		...
		// Skip if outside of the light range
		if (A <= 0.0)
			continue;
